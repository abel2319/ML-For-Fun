...
#--------------Abeel add target_size=None----------------------------------------------------
def load_image_features(name, features, target_size=None):
  images = []
  num_channels = {}

  # HDR color
  if 'hdr' in features:
    hdr, num_channels['hdr'] = load_image(name + '.hdr.exr', num_channels=3, target_size=target_size)
    hdr = np.maximum(hdr, 0.)
    images.append(hdr)

  # LDR color
  if 'ldr' in features:
    ldr, num_channels['ldr'] = load_image(name + '.ldr.exr', num_channels=3, target_size=target_size)
    ldr = np.clip(ldr, 0., 1.)
    images.append(ldr)
  ...
  # Albedo
  if 'alb' in features:
    albedo, num_channels['alb'] = load_image(name + '.alb.exr', num_channels=3, target_size=target_size)
    albedo = np.clip(albedo, 0., 1.)
    images.append(albedo)

  # Normal
  if 'nrm' in features:
    normal, num_channels['nrm'] = load_image(name + '.nrm.exr', num_channels=3, target_size=target_size)
  ...
  # Depth
  if 'z' in features:
    depth, num_channels['z'] = load_image(name + '.z.exr', num_channels=1, target_size=target_size)
